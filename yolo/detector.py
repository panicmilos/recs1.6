...
def on_release(key):
    if str(key) == 'Key.f8':
        global should_aimbot
        should_aimbot = not should_aimbot
        if should_aimbot:
    ...
    if str(key) == 'Key.f9':
        global FLAGS
        FLAGS.team = 'ct' if FLAGS.team == 'tt' else 'tt'
        if FLAGS.team == 'ct':
            print('Team switched to ct.')
        else:
            print('Team switched to tt.')
...
def execute_as_aimbot():
    ...
    while True:
        ...
        prediction = detect_object_wo_io(yolo, image)
        for single_prediction in prediction:
            team_num = 0 if FLAGS.team == 'ct' else 1
            if single_prediction[4] == team_num or single_prediction[5] < 0.5:
                continue

            x = single_prediction[0] + (single_prediction[2] - single_prediction[0]) / 2
            y = single_prediction[1] + (single_prediction[3] - single_prediction[1]) / 4
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--aimbot",
        default=False,
        help="Use this to kill people. Default is False.",
    )

    parser.add_argument(
        "--team",
        default='ct',
        help="Specify client team. Default is ct.",
    )
